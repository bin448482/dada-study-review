"""Load one verified, non-discoverable Dialogue state-machine definition."""
...
_EXPECTED_FILES = frozenset({"SKILL.md", "agents/openai.yaml", "references/dialogue-turn-contract.md"})
...
def load_state_machine_definition(directory: Path, definition_id: str, expected_digest: str) -> StateMachineDefinition:
    if definition_id != "dada-dialogue-state-machine" or not re.fullmatch(r"[a-f0-9]{64}", expected_digest):
    ...
    for path in _bundle_files(root):
        try:
            ...
            raise DefinitionError("definition file is not UTF-8") from exc
    joined = "\n".join(texts.values())
    if "dada.dialogue_state_machine_turn" not in joined or "https://" in joined or "http://" in joined or _CREDENTIAL_SHAPE.search(joined):
    ...
    return StateMachineDefinition(
        definition_id, actual, "\n\n".join((texts["SKILL.md"], texts["references/dialogue-turn-contract.md"])),
        {"type": "object", "additionalProperties": False, "required": ["contract_name", "contract_version", "data"],
         "properties": {"contract_name": {"const": "dada.dialogue_state_machine_result"}, "contract_version": {"const": 1}, "data": {"type": "object"}}},
    )
