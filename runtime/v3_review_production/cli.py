...
_REQUIRED_ENV = (
    ...
    "DADA_REVIEW_MODEL_API_STYLE", "DADA_REVIEW_MODEL_API_KEY", "DADA_REVIEW_SCHEDULE_PATH",
...
def run(value: Any) -> dict[str, object]:
    ...
    service = build_review_service(
        ...
        ProductionGatewayConfig(environment["DADA_REVIEW_MODEL_PROVIDER"], environment["DADA_REVIEW_MODEL"], environment["DADA_REVIEW_MODEL_ENDPOINT"], 120, environment["DADA_REVIEW_MODEL_API_STYLE"]),
        UrllibBearerResponsesTransport(environment["DADA_REVIEW_MODEL_API_KEY"]), ReviewSchedulePolicy.from_file(Path(environment["DADA_REVIEW_SCHEDULE_PATH"])),
    ...
    try:
        ...
        return {"ok": True, "handled": delivery.handled, "reply_text": delivery.reply_text, "no_due_item": delivery.no_due_item}
    finally:
        service.close()
...
def main() -> int:
    ...
    except (ValueError, OSError, json.JSONDecodeError): result = {"ok": False, "handled": False, "reply_text": None, "no_due_item": False}
