...
def validate_review_state_machine_result(value: Any, selected_question_mode: str | None = None) -> ReviewStateMachineResult:
    ...
    if envelope["contract_name"] != "dada.review_state_machine_result" or envelope["contract_version"] not in (1, 2, 3):
    ...
    if operation == "ask_question":
        if envelope["contract_version"] == 3:
            ...
            return ReviewStateMachineResult("ask_question", question_mode=question_mode, question_json=_question_json(item["question_json"]))
    ...
    if operation == "request_transition":
        if envelope["contract_version"] not in (2, 3):
...
def _question_json(value: Any) -> dict[str, str]:
    item = _object(value, "question json")
    if set(item) - {"prompt", "instruction"} or "prompt" not in item:
...
def render_locked_question(question_json: dict[str, Any]) -> str:
    ...
    question = _question_json(question_json)
